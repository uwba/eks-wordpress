...
class VA_Widget_Create_Listing_Button extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		echo $before_widget;
		echo html_link( $url, __( 'Add your Tax Site now', APP_TD ) );
		echo $after_widget;
	}
}
...
class VA_Widget_Listing_Map extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		APP_Geo_Query::enqueue_scripts( 'vantage_map_view' );
	}
		...
		if ( $reviews ) {
			foreach ( (array) $reviews as $review ) {
				...
				$output .=  '<li class="recent-review clear">' .
					...
					html_link( va_get_the_author_reviews_url( $user->ID ) , get_avatar( $user, 45 ) ) .
					'</div>'.
