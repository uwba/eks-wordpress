...
Description: Volunteer registration and management plugin
Author: Andrey
Version: 1.0
Author URI:
*/
...
//	if ($_POST['action'] == 'myajax_submit') {
	switch ($step) {
		...
		case 2: // Position
			$_SESSION['volunteer']['preparer'] = $wpdb->escape($_POST['preparer']);
//			$_SESSION['volunteer']['position'][0] = $wpdb->escape($_POST['position']);
			$_SESSION['volunteer']['position'] = $_POST['position'];
			// validate fields
			if ($_SESSION['volunteer']['position'][0] == 'preparer'
					&& !$_SESSION['volunteer']['preparer']) {
				$errors[] = 'Please select your experience';
			}
			$valid = !count($errors);
...
//			var_dump($_SESSION['volunteer']);
			break;
			
		case 3: // Location/Tax site			
//			$_SESSION['volunteer']['position'] = $_POST["position"];
			$_SESSION['volunteer']['tax_sites'][0] = $_POST['tax_sites'];
			$valid = !count($errors);
			// Training: Classroom training, Link and Learn, Ethics
			// Roles
			// New Tax Preparer (volunteered in another capacity other than preparer)
			// Tax Preparer
			// Tax Professional
			// Greeter
			// Screener
...
//			$positions = array();
//			$counties = array();
//			foreach ($_SESSION['volunteer']['position'] as $id => $tax_site) {
//				$tax_sites[] = $id;
//				foreach ($tax_site as $position) {
//					$positions[] = $position;
//				}
//			}
//			$positions = array_unique($positions);
			
			$positions = $_SESSION['volunteer']['position'];
			$tax_sites = $_SESSION['volunteer']['tax_sites'];
			
			$new = $_SESSION['volunteer']['preparer'];
			$position = implode(', ', $positions);//$_SESSION['volunteer']['position']
			
			$args = array( 'numberposts' => -1, 'post_type' => 'listing', 'post_status' => 'publish', 'post__in' => $tax_sites);
			$myposts = get_posts( $args );
			$training_types = array(); // TODO: add tax site id to each training type
			foreach ($myposts as $post) {
				$county = get_the_listing_category($post->ID);
				$counties[] = $county->name;
				
				if (in_array('preparer', $positions)){
					if ($new == 'new') {
						$training_types = array_merge($training_types, get_post_meta($post->ID, 'app_newtaxpreparertrainingrequiredcheckallthatapply'));
					} else { // returning tax preparer
						$training_types = array_merge($training_types, get_post_meta($post->ID, 'app_returningtaxpreparertrainingrequiredcheckallthatapply'));
					}
				}
			}
			...
			$main_county = $county->name; 
			$county = implode(', ', $counties);
			
			
			
			$html = "Since you've selected that you are {$new} {$position} volunteering at {$county} please choose from these training options:<br/><br/>";
			

		
			
			if ($main_county == 'Marin') {
				$training_types[] = 'Info Nights';
			}
			
			$training_types[] = 'Ethics';	
			$training_types = array_unique($training_types);
//			var_dump($training_types); exit;
...
//			var_dump($results);
			...
			foreach($training_types as $training_type) {
				$html .= "<h3>{$training_type}</h3>";
				$count = 0;
				foreach ($results as $training) {
					// TODO: check tax site id for each training type
					$training_names = unserialize($training->training);
//					var_dump($training_names);
					foreach ($training_names as $training_name){
						if ($training_type == $training_name) {
	//						echo 'aaaaaaaaaaaaaaaa';
//							$out = current(unserialize($training->training));
	//						$html .= "<h3>{$out}</h3>";
							$html .= "<label><input type='radio' name='training[{$training_name}]' value='{$training->ID}'/>{$training->post_title}</label></br>";
							$count++;
						}
					}
				...
				if (!$count) {
					$html .= "<p>No trainings found. Please contact your coordinator</p>";
				}
			}//exit;
...
//			$training_type = '';
//			foreach ($results as $training) {
//				if ($training_type != $training->training) {
//					$training_type = $training->training;
//					$out = current(unserialize($training_type));
//					$html .= "<h3>{$out}</h3>";
//				}
//				$html .= "<label><input type='radio' name='training[{$out}]' value='{$training->ID}'/>{$training->post_title}</label></br>";
//			}
		...
		case 4: // Training
			...
			$my_post = array(
			  ...
			  // 'post_content' => $_POST['post_content'],
			   ...
			   'post_author' => $user_ID ? $user_ID : $_SESSION['volunteer']['user_ID'],
			);
	...
	?><div class="form-field mti_font_element">
		<label class="mti_font_element">
			...
			<select id="role" name="role" tabindex="5">
				...
				<option <?php if (isset($_GET['role']) && $_GET['role'] == 'volunteer') {
		echo 'selected';
	} ?> value="volunteer">Volunteer</option>
				<option <?php if (isset($_GET['role']) && $_GET['role'] == 'coordinator') {
		echo 'selected';
